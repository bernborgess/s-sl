...
/* Keyword Token Table */
char *keywordText[] = {"mechanism", "output", "input", "error", "rules", "type",
                       "end",       "do",     "od",    "if",    "fi"};

InputTokens keywordToken[] = {tMechanism, tOutput, tInput,    tError,
                              tRules,     tType,   tEnd,      tCycle,
                              tCycleEnd,  tChoice, tChoiceEnd};
...
int main(int argc, char *argv[]) {
  ...
  if (errflag || argc < optind + 1) {
    fprintf(stderr,
            "Usage: %s [-TLs] [-E Name] [-D type] [-h file.sst.h] [-t "
            "file.sst.c] [-l file.lst] [-e file.entry] [-i] file.ssl\n",
            progname);
    exit(1);
  }
  ...
  if (summarize)
    printf("%d/%d table entries, %d/%d symbols, %d/%d calls.\n",
           outputPointer + 1, maxOutputTableSize, symTop, maxSymbols, callTop,
           maxCalls);
  exit(0);
}
...
char *LowerCase(s) register char *s;
{
...
void Error(errCode) ErrorCodes errCode;
{
  if (errCode == eNoError)
    assertionFailure;
  ...
  if (((int)errCode) >= ((int)firstFatalErrorCode) || noErrors == maxErrors) {
...
void GetNextInputToken() {
  ...
  while (nextChar == blank || nextChar == tab || nextChar == newpage ||
         nextChar == '%') {
  ...
  if (isascii(nextChar) && isalpha(nextChar)) {
    ...
    do {
      ...
      ReadNextChar();
    } while (nextChar != quote && nextChar != newline && nextChar != endfile);
...
void AcceptInputToken() {
  ...
  if (acceptedToken == tIdent || acceptedToken == tString ||
      acceptedToken == tInteger) {
...
void EmitFixup(fixupAddress, value) OutputAddress fixupAddress;
int value;
{
  if (fixupAddress >= outputPointer || outputTable[fixupAddress] != nullAddress)
    assertionFailure;
...
void EmitNullAddress() { Emit(nullAddress); }

/* Emit a backward jump address.  Jump addresses are absolute. */

void EmitJumpAddress(targetAddress) OutputAddress targetAddress;
{ Emit(targetAddress); }

/* Fixup a forward jump address.  Jump addresses are absolute. */

void EmitJumpFixup(jumpAddress) OutputAddress jumpAddress;
{ EmitFixup(jumpAddress, outputPointer); }

void EmitNullCallAddress() { EmitNullAddress(); }
...
void LookupSymbol() {
  ...
  while (
      symIndex != notFound &&
      (h != symHash[symIndex] || strcmp(ct, LowerCase(symText[symIndex])) != 0))
    --symIndex;
...
void EnterSecondNewSymbolValue() {
  if (symTop < 2)
    assertionFailure;
...
void EnterSymbolValueFromAddress() { symValue[symIndex] = outputPointer; }

void SetCurrentSymbol(newIndex) SymbolIndex newIndex;
{ symIndex = newIndex; }

void xSetCurrentClass(newClass) SymbolClass newClass;
{
  int nextOpValue;

  /* Synchronize operation values */
  if (symCurrentClass == cUpdateOp || symCurrentClass == cParmUpdateOp ||
      symCurrentClass == cChoiceOp || symCurrentClass == cParmChoiceOp) {
  ...
  if ((symCurrentClass == cInput || symCurrentClass == cOutput) &&
    ...
    symNextValue[(int)cInputOutput] = symNextValue[(int)symCurrentClass];

  symCurrentClass = newClass;
}

void SetNextValueOfCurrentClass(newValue) int newValue;
{ symNextValue[(int)symCurrentClass] = newValue; }

void IncrementCurrentClassValue() { symNextValue[(int)symCurrentClass] += 1; }

void CopySymbolTextToTokenBuffer() {
  (void)strcpy(compoundText, symText[symIndex]);
}

void ChangeSymbolClass() {
...
void xEnterSymbolParameterClass() {
  if (symIndex == notFound)
    assertionFailure;
...
void SaveCurrentSymbol() { savedSymIndex = symIndex; }

void RestoreCurrentSymbol() { symIndex = savedSymIndex; }

void SaveEnclosingSymbol() { enclosingSymIndex = symIndex; }

void RestoreEnclosingSymbol() { symIndex = enclosingSymIndex; }
...
void EmitCycleAddress() { EmitJumpAddress(cycleAddress[cycleTop]); }
...
void PushChoice(pushClass) SymbolClass pushClass;
{
...
void xEnterChoiceLabel(value) int value;
{
  if (choiceTop == 0)
    assertionFailure;
...
Validity VerifyChoiceLabel(value) int value;
{
  u_int i;
...
void xResolveChoiceTableAddress() {
  if (choiceTop == 0)
    assertionFailure;
...
void ChangeChoiceClass(newClass) SymbolClass newClass;
{ choiceClass[choiceTop] = newClass; }

void InitChoiceStack() {
  mergeTop = 0;
  labelTop = 0;
  choiceTop = 0;
}
...
void GenerateClass(class, name) SymbolClass class;
char *name;
{
  SymbolIndex s;
  SymbolClass c;
  ...
  for (s = 0; s != symTop;) {
    ...
    if ((c == class ||
         (class == cUpdateOp &&
          (c == cParmUpdateOp || c == cChoiceOp || c == cParmChoiceOp))) &&
        symText[s][0] != quote) {
      ...
      if (symValue[s] != lastvalue + 1) {
        (void)sprintf(buf, "%s = %d", symText[s], symValue[s]);
      } else
        (void)strcpy(buf, symText[s]);
      fprintf(headerfp, "\t%s,%s/* %d */\n", buf,
              "\t\t\t\t\t\t" + (strlen(buf) + 1) / 8, symValue[s]);
...
void GenerateOutputTable(typeName) char *typeName;
{
  OutputAddress i;
...
void GenerateEntryPoints(enumName) char *enumName;
{
  SymbolIndex s;
...
void SslGenerateCompoundInputToken(expectedToken) InputTokens expectedToken;
{
...
void SslSyntaxError() {
  ...
  if (nextToken == tSyntaxError) {
    ...
    if (sslTable[sslPointer] == (int)tNewLine ||
      ...
      newInputLine = false;
      while (nextToken != tSemicolon && nextToken != tEndOfFile && newInputLine)
        AcceptInputToken();
    }
  } else {
...
void SslFailure(failCode) FailureCodes failCode;
{
...
void SslChoice(choiceTag) int choiceTag;
{
  int numberOfChoices;
  u_int choicePointer;
