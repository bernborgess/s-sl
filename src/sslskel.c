...
STATIC void Error(errCode) ErrorCodes errCode;
{
  char *msg;
...
STATIC void EmitOutputToken(emittedToken) OutputTokens emittedToken;
{
...
STATIC void
    SslGenerateCompoundInputToken(expectedToken) InputTokens expectedToken;
{
...
STATIC void SslSyntaxError() {
  ...
  if (nextInputToken == tSyntaxError) {
    ...
    if (sslTable[sslPointer] == (int)(tNewLine) ||
      ...
      newInputLine = 0;
      while (nextInputToken != tSemicolon && nextInputToken != tEndOfFile &&
             !newInputLine)
        AcceptInputToken();
    }
  } else {
...
STATIC void SslFailure(failCode) FailureCodes failCode;
{
...
STATIC void SslChoice(choiceTag) int choiceTag;
{
  ...
  for (numberOfChoices = sslTable[choicePointer++]; numberOfChoices > 0;
